...
def writeTFRecord(path, filename):
    ...
    with open(path + filename + path_size + '.txt', 'w') as f:  # txt文件生成地址
        train_file_len = len(os.listdir(path + 'label'))  # 样本个数
        for i in range(1, train_file_len + 1):
            with open(path + 'label/' + str(i) + '.txt', 'r') as fl:
                label = fl.read()
            # images路径和labels用，做区分
            f.write(path + 'img' + path_size + '/'
                    + str(i) + '.jpg' + ',' + label + '\n')

    # 生成list，用于生成tfrecords文件
    with open(path + filename + path_size + '.txt', 'r') as f:
        for line in f.readlines():
            images_list.append(line.split(',')[0])  # images路径列表
            labels_list.append(line.split(',')[1])  # labels列表

    # 生成tfrecords文件
    writer = tf.python_io.TFRecordWriter(path + filename + path_size + '.tfrecords')
